% User defined function to compute spectrogram with moving window Hamming, Hanning.
% Author: Rinu Sebastian
% Date: 11/26/2018
% Status: can depend fully

function [mat_psd_ham,mat_psd_han,PSD_ham,PSD_han,off_set,time,freq]=specto(signal,channel_num,time_pts,overlap_per,Fs)
M=1000; %1 second window
N=2^nextpow2(M); 
off_set=overlap_per*M/100;
% t_pts=size(signal,2);
f_pts=N/2+1;
freq=0:Fs/(2*f_pts):Fs/2;
