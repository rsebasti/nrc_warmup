...
% data_dir = 'D:\Research\Other\Data\Kai Miller\mot_t_h\data'; % specify location of data file
...
h0=figure('Position',[50 100 800 1200]);
...
bottom_margin = 0.04; % bottom spacing on the figure
right_margin = 0.04; % right spacing on the figure
top_margin = 0.04; % top spacing on the figure
ax_spacing_horizontal = 0.01; % spacing between columns of axes
ax_spacing_vertical = 0.02; % spacing between rows of axes
...
for ii = rows:-1:1
    for jj = 1:cols
        plot(ax(ii,jj),t,ecog_ data(:,rows-ii+1));
...
title(ax(rows,1),"Sample Graph Amplitude vs time - Channels 1 to 10");
...
for kk=1:channel_num
    ...
    for mm=row1:-1:1
        ...
        ylabel(ax1(mm,nn),ylab2);
        if mm>1
            set(ax1(mm,nn),'XTickLabel',[]);
        else 
            xlabel(ax1(mm,nn),'Time (sec)');
        end
        end
    end
    hold on;
